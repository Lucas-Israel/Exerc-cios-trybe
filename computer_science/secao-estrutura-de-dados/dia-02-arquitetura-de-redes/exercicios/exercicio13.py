# No conteúdo vimos o que são os protocolos SSL e TLS. Vamos subir nosso
# próprio servidor HTTPS, utilizando nosso próprio certificado!
...
# ele não será reconhecido por nenhuma entidade certificadora, de modo que ele
# só nos servirá para utilizar o protocolo TLS com o HTTPS, não sendo capaz de
# ser aceito pelo navegador por não ter sido aprovado por nenhuma entidade
# reconhecida por ele.
...
# (chave privada). Copie os dois arquivos para um diretório onde vamos criar
# nosso servidor HTTPS.

# 4. Agora vamos escrever um servidor https usando os módulos nativos do python
#  ssl e http.server. Embora esses módulos tenham muitos recursos
# (muitos mesmo), nós vamos usar apenas alguns. Tente seguir as
# instruções a seguir:

# 4.1 Crie um contexto SSL com a classe SSLContext, usando o protocolo de
# versão mais alta disponível para servidores.
# (dica: as opções estão listadas na documentação).

# 4.2 Carregue no contexto SSL a cadeia de certificação, passando tanto
# o arquivo de certificação quanto a sua chave (dica: existe um método
# para isso).
...
# (https://stackoverflow.com/a/60516812/207119), mas não precisa seguir
# por esse caminho.

# 6. Acesse o servidor novamente, porém desta vez utilizando cURL (de fora do
# Docker, se você estiver usando).
...
# parâmetro -k ou --insecure. Com ele, falamos para o nosso cURL prosseguir
# a request mesmo sabendo que a conexão não é “confiável”.
